  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setSentMessages((prev) => [
        ...
        {
          sender: host?.name || "Host",
          text: "Thanks for reaching out! I typically respond within an hour. Looking forward to hosting you!",
          time: "Just now",
          fromHost: true,
        },
      ]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-pink-50 px-6 py-2 border-b border-pink-100 flex items-center justify-between text-xs text-gray-700">
          <span className="flex items-center gap-1.5 font-medium text-[#FF385C]">
            <Sparkles className="w-3.5 h-3.5" /> Guest-to-Host Messaging
          </span>
          <span className="text-[11px] bg-white border border-pink-200 text-gray-600 px-2 py-0.5 rounded-full font-semibold">
            Demo Mode (Real-time socket chat coming soon)
          </span>
        </div>
        ...
        <div className="px-6 py-2 border-t border-gray-100 bg-white flex flex-wrap gap-1.5">
          {quickQuestions.map((q, i) => (
            <button
              key={i}
              type="button"
              onClick={() => setMessage(q)}
              className="text-[11px] bg-gray-100 hover:bg-gray-200 text-gray-700 px-2.5 py-1 rounded-full transition cursor-pointer"
            >
              {q}
            </button>
          ))}
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-gray-200 bg-white flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-gray-300 px-3.5 py-2.5 text-sm focus:border-black outline-hidden"
          />
          <button
            type="submit"
            disabled={!message.trim()}
            className="rounded-xl bg-[#FF385C] px-4 py-2.5 text-white font-bold text-xs hover:bg-[#E00B41] transition disabled:opacity-40 cursor-pointer flex items-center gap-1.5"
          >
